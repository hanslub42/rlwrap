...
#    define snprintf4(buf, bufsize, format, arg1, arg2, arg3, arg4) \
        snprintf(buf, bufsize, format, arg1, arg2, arg3, arg4)
#    define snprintf5(buf, bufsize, format, arg1, arg2, arg3, arg4, arg5) \
        snprintf(buf, bufsize, format, arg1, arg2, arg3, arg4, arg5)
#    define snprintf6(buf, bufsize, format, arg1, arg2, arg3, arg4, arg5, arg6) \
        snprintf(buf, bufsize, format, arg1, arg2, arg3, arg4, arg5, arg6)
...
#    define snprintf6(buf, bufsize, format, arg1, arg2, arg3, arg4, arg5, arg6) \
        sprintf(buf, format, arg1, arg2, arg3, arg4, arg5, arg6)
...
char  *search_and_replace(char *patt, const char *repl, const char *string, int cursorpos, int *line, int *col);
...
extern char          *term_backspace, term_eof, term_stop, *term_cursor_hpos,
					 *term_cursor_up, *term_cursor_down,*term_cursor_left,
					 *term_smcup, *term_rmcup, *term_rmkx,
					 *term_enable_bracketed_paste, 	*term_disable_bracketed_paste;
...
typedef enum {
    TEST_AT_PROGRAM_START,
    TEST_AFTER_OPTION_PARSING,
    TEST_AFTER_SPAWNING_SLAVE_COMMAND,
    TEST_AFTER_READLINE_INIT
} test_stage;

#define TESTFUNC(f, argc, argv, stage) void f(int argc, char **argv, test_stage stage)
#define ONLY_AT_STAGE(s) \
    if (stage != s) return
...
#define KA_SCREECH                                                                           \
    kill(getpid(), SIGTRAP); /* enter the debugger - use it to set (conditional) breakpoints \
            from within C code: if (condition) KA_SCREECH; */

/* DPRINTF0 and its ilk  doesn't produce any output except when DEBUG is #defined
    (via --enable-debug configure option) */

/* Use a superfluous "break" (withn a switch) to prevent "this statement may fall through" warnings
   when we know that a statement will end the program anyway */
#define WONTRETURN(statement) \
    statement;                \
    break
...
#    define MANGLE_LENGTH \
        ((debug_saved & DEBUG_LONG_STRINGS) ? 0 : 20) /* debug_saved is defined within DPRINTF macro */
...
#    define WHERE_AND_WHEN                                                        \
        ...
        fprintf(debug_fp, "%-20s %s %-7s %-25.25s ", file_line, when,             \
                (i_am_child    ? "child"                                          \
                 : i_am_filter ? "filter"                                         \
                               : "parent"),                                       \
                __MYFUNCTION__);
...
#    define NL_AND_FLUSH                  \
        fputc('\n', debug_fp);            \
        fflush(debug_fp);                 \
        flock(fileno(debug_fp), LOCK_UN); \
        debug = debug_saved

#    define DPRINTF0(mask, format)            \
        do {                                  \
            if ((debug & mask) && debug_fp) { \
                WHERE_AND_WHEN;               \
                fprintf(debug_fp, format);    \
                NL_AND_FLUSH;                 \
            }                                 \
        } while (FALSE)
...
#    define DPRINTF2(mask, format, arg1, arg2)         \
        do {                                           \
            if ((debug & mask) && debug_fp) {          \
                WHERE_AND_WHEN;                        \
                fprintf(debug_fp, format, arg1, arg2); \
                NL_AND_FLUSH;                          \
            }                                          \
        } while (FALSE)
...
#    define DPRINTF3(mask, format, arg1, arg2, arg3)         \
        do {                                                 \
            if ((debug & mask) && debug_fp) {                \
                WHERE_AND_WHEN;                              \
                fprintf(debug_fp, format, arg1, arg2, arg3); \
                NL_AND_FLUSH;                                \
            }                                                \
        } while (FALSE)
...
#    define DPRINTF4(mask, format, arg1, arg2, arg3, arg4)         \
        do {                                                       \
            if ((debug & mask) && debug_fp) {                      \
                WHERE_AND_WHEN;                                    \
                fprintf(debug_fp, format, arg1, arg2, arg3, arg4); \
                NL_AND_FLUSH;                                      \
            }                                                      \
        } while (FALSE)
...
#    define DPRINTF5(mask, format, arg1, arg2, arg3, arg4, arg5)         \
        do {                                                             \
            if ((debug & mask) && debug_fp) {                            \
                WHERE_AND_WHEN;                                          \
                fprintf(debug_fp, format, arg1, arg2, arg3, arg4, arg5); \
                NL_AND_FLUSH;                                            \
            }                                                            \
        } while (FALSE)
...
#    define SHOWCURSOR(c)               \
        if (debug & DEBUG_SHOWCURSOR) { \
            my_putchar(c);              \
            mymicrosleep(1000);         \
            curs_left();                \
        } /* (may work incorrectly at last column!)*/

#    define DEBUG_RANDOM_SLEEP                                         \
        if (debug & DEBUG_RACES) {                                     \
            int sleeptime = rand() & 31;                               \
            DPRINTF1(DEBUG_RACES, "sleeping for %d msecs", sleeptime); \
            mymicrosleep(sleeptime);                                   \
        }
