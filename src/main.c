...
char *extra_char_after_completion = " ";    /* -e option: override readlines's default
                                                  completion_append_char (space) */
...
int   impatient_prompt = TRUE;  /* show raw prompt as soon as possible, even before we cook it.
                                    may result in "flashy" prompt */
char *substitute_prompt = NULL; /* -S option: substitute our own prompt for <command>s */
char *filter_command = NULL;    /* -z option: pipe prompts, input, output, history and completion
                                    requests through an external filter */
...
char *program_name = "rlwrap",
     *command_name;                /* "rlwrap" (or whatever has been symlinked to rlwrap) and (base-)name of command */
...
int   ignore_queued_input = FALSE; /* read and then ignore all characters in input queue until it
                                      is empty (i.e. read would block) */
int received_WINCH = FALSE;        /* flag set in SIGWINCH signal handler: start line edit as soon as possible */
int prompt_is_still_uncooked =
    TRUE; /* The main loop consults this variable to determine the select() timeout: when TRUE, it is
...
int we_just_got_a_signal_or_EOF = FALSE; /* When we got a signal or EOF, and the program sends
                                            something that ends in a newline, take it as a
                                            response to user input - i.e. preserve a cooked prompt
                                            and just print the new output after it */
int  rlwrap_already_prompted = FALSE;
int  accepted_lines = 0;              /* number of lines accepted (used for one-shot rlwrap) */
bool user_has_typed_first_NL = FALSE; /* When the *first* user NL is typed in direct mode, we probably need
                                         --always-readline: give a warning, .. */
...
/* helper function to run a unit test whenever UNIT_TEST is #defined, e.g. by "make clean; make
 * CFLAGS='-DUNIT_TEST=my_test'" example of such an unit test (can be defined in any of the .c files):
 *
...
#else
static void run_unit_test(int UNUSED(argc), char **UNUSED(argv), test_stage UNUSED(stage)) { /*do nothing */ }
#endif
...
int main(int argc, char **argv)
{
    ...
    if (!(setlocale(LC_ALL, "") && setlocale(LC_COLLATE, "C"))) { /* ANSI C says that all programs
                                                                     start by default in the standard `C' locale...   */
        myerror(WARNING | NOERRNO, "could not set locale");       /* ... To use the locales
                                                      specified by the environment, we must call  setlocale. */
    }
    ...
    run_unit_test(argc - optind, argv + optind,
                  TEST_AFTER_OPTION_PARSING); /* argv points at the first non-option rlwrap argument */
    ...
    if (filter_command) spawn_filter(filter_command);
    run_unit_test(argc - optind, argv + optind, TEST_AFTER_SPAWNING_SLAVE_COMMAND); /* argv points
                at the first non-option rlwrap argument */
 ...
 */
static void fork_child(char *UNUSED(command_name), char **argv)
{
    char *arg = argv[optind], *p;
    int   pid;

    if (mirror_arguments) mirror_args_init(&argv[optind]);
    ...
    if (pid > 0) /* parent: */
        ...
        if (client_term_name) mysetenv("TERM", client_term_name);
        if (execvp(argv[optind], &argv[optind]) < 0) {
            if (last_opt > 0 && last_option_didnt_have_optional_argument) { /* e.g. 'rlwrap -a
                    Password: sqlpus' will try to exec 'Password:' */
            ...
            myerror(FATAL | USE_ERRNO, "Cannot execute %s", argv[optind]); /* stillborn child,
                        parent will live on and display child's last gasps */
        }
    }
}
...
void main_loop(void)
{
    ...
    while (TRUE) {
        ...
        FD_ZERO(&writefds);
        if (output_queue_is_nonempty()) FD_SET(master_pty_fd, &writefds);
        ...
        /*          100*(FD_ISSET(STDIN_FILENO, &readfds)?1:0) + 10*(FD_ISSET(master_pty_fd, &readfds)?1:0) +
         * (FD_ISSET(master_pty_fd, &writefds)?1:0),  */
        /*          within_line_edit); */
        ...
        if (filter_is_dead) filters_last_words(); /* will call myerror with last words */
        ...
        if (nfds < 0) {                         /* exception  */
            ...
            if (ignore_queued_input) { /* ... we have read all the input keystrokes that should
                ...
                if (we_just_got_a_signal_or_EOF) {
                    we_just_got_a_signal_or_EOF = FALSE; /* 1. If we got a signal/EOF before
                                                               cooking time, we don't need
                                                               special action to preserve the
                                                               cooked prompt.
                                                            2. Reset we_just_got_a_signal_or_EOF
                                                               after a signal or EOF
                                                               that didn't kill command */
                    continue;
                }
                if (!skip_rlwrap()) { /* ... or else, it is time to cook the prompt */
                    ...
                    move_cursor_to_start_of_prompt(ERASE); /* cooked prompt may be shorter
                        than raw prompt, hence the ERASE move and erase before cooking,
                        as we need to move/erase according to the raw prompt */
                ...
                if (mirror_arguments) mirror_args(command_pid);
                continue;
            } else {
            ...
            /* Always first read and process the slave command's output, even if there is input waiting on stdin
               ...
               c
            */
            if (FD_ISSET(master_pty_fd, &readfds)) {            /* there is something (or nothing,
                                                                    if EOF) to read on master pty: */
                nread = read(master_pty_fd, buf, BUFFSIZE - 1); /* read it */
                DPRINTF1(DEBUG_AD_HOC, "nread: %d", nread);
                if (nread <= 0) {
                    if (command_is_dead || nread == 0) { /* we catched a SIGCHLD, or slave
                                                            command has closed its stdout */
                        if (promptlen > 0)               /* commands dying words were not terminated by \n ... */
                            my_putchar('\n');            /* provide the missing \n */
                        cleanup_rlwrap_and_exit(EXIT_SUCCESS);
                    } else if (errno == EINTR) { /* interrupted by signal ...*/
                        continue;                /* ... don't worry */
                    } else if (!seen_EOF) {      /* maybe command has just died (and SIGCHLD,
                                                    whose handler sets command_is_dead is not  */
                ...
                check_cupcodes(buf);
                if (skip_rlwrap()) { /* Race condition here! The client may just have finished an
                                        emacs session and returned to cooked mode, while its
                                        ncurses-riddled output is stil waiting for us to be
                                        processed. */
                    if (advise_always_readline) {
                        ...
                        myerror(WARNING | NOERRNO,
                                "rlwrap appears to do nothing for %s, which asks for\n"
                ...
                write_logfile(buf);
                if (within_line_edit) { /* client output arrives while we're keyboardinput:  */
                    save_rl_state();    /* temporarily disable readline and restore the screen
                            state before readline was called */
                }
                ...
                */
                ...
                /* sometimes we want to leave the prompt standing, e.g. after accepting a line, or
                   when a signal arrived */
                // clang-format off
                leave_prompt_alone = *saved_rl_state.raw_prompt == '\0' /* saved_rl_state.raw_prompt = "" in two
                                            distinct cases: when there is actually no prompt,
                                            or just after accepting a line, when the cursor is at the end of the prompt.
                                            In both cases, we dont't want to move the cursor */
                                     || prompt_is_still_uncooked /* in this case no prompt has been displayed yet */
                                     || command_is_dead
                                     ||(we_just_got_a_signal_or_EOF&& strrchr(buf, '\n'));  /* a
                                        signal followed by output with a newline in it:
                                        treat it as response to user input,
                                        so leave the prompt alone */
                // clang-format on
                ...
                if (impatient_prompt && !leave_prompt_alone) old_raw_prompt = mysavestring(saved_rl_state.raw_prompt);

                new_output_minus_prompt = process_new_output(buf, &saved_rl_state); /* chop off
                        the part after the last newline and put this in
                        saved_rl_state.raw_prompt (or append buf if  no newline found) */

                if (impatient_prompt) { /* in impatient mode, ALL command output is passed through
                                           the OUTPUT filter, including the prompt The prompt,
                                           however, is filtered separately at cooking time and
                                           then displayed */
                    ...
                    if (regexp_means_prompt && prompt_regexp && match_regexp(saved_rl_state.raw_prompt, prompt_regexp, FALSE)) {
                        /* user specified -O!.... so any natching candidate prompt will be cooked
                ...
                if (within_line_edit) restore_rl_state();

                yield();  /* wait for what client has to say .... */
                continue; /* ... and don't attempt to process keyboard input as long as it is
                             talking , in order to avoid re-printing the current prompt
                             (i.e. unfinished output line) */
            }

            /* ----------------------------- key pressed: read stdin -------------------------*/
            if (FD_ISSET(STDIN_FILENO, &readfds)) {        /* key pressed */
                unsigned char byte_read;                   /* the readline function names and
                    documentation talk about "characters" and "keys", but we're reading
                    bytes (i.e. unsigned chars) here, and those may very well be part of a
                    multi-byte character. Example: hebrew "aleph" in utf-8 is 0xd790;
                    pressing this key will make us read 2 bytes 0x90 and then 0xd7, (or maybe the
                    other way round depending on endianness??) The readline library hides all this
                    complexity and allows one to just "pass the bytes around" */
                nread = read(STDIN_FILENO, &byte_read, 1); /* read next byte of input   */
                assert(sizeof(unsigned char) == 1);        /* gets optimised away       */

                if (nread <= 0) DPRINTF1(DEBUG_TERMIO, "read from stdin returned %d", nread);
                if (nread < 0)
                    if (errno == EINTR)
                        continue;
                    else
                ...
                if (skip_rlwrap()) { /* direct mode, just pass it on */
                    ...
                    if (!user_has_typed_first_NL && (byte_read == '\r' || byte_read == '\n')) {
                        ...
                        advise_always_readline = TRUE; /* first NL is in direct mode:
                                                          advise the user that she probably wants
                                                          --always-readline */
                    }

                    completely_mirror_slaves_terminal_settings(); /* this is of course 1 keypress too late: we should
                                                                     mirror the terminal settings *before* the user
                                                                     presses a key. (maybe using rl_event_hook??)
                                                                     @@@FIXME  @@@ HOW?*/
...
static void init_rlwrap(char *command_line)
{
    ...
    homedir_prefix = (getenv("RLWRAP_HOME") ? /* is RLWRAP_HOME set?                */
                          add2strings(getenv("RLWRAP_HOME"), "/")
    ...
    if (write_histfile) {
        if (access(history_filename, F_OK) == 0) { /* already exists, can we read/write it? */
            ...
            if (statbuf.st_uid != geteuid()) {
                myerror(WARNING | NOERRNO, "Owner of %s and your effective UID don't match. History will be read-only",
                        history_filename);
    ...
    read_history(history_filename); /* ignore errors here: history file may not yet exist, but
                                       will be created on exit */

    if (feed_history_into_completion_list) feed_file_into_completion_list(history_filename, FALSE);
    ...
    default_completion_filename = add3strings(DATADIR, "/rlwrap/completions/", command_name);
 ...
 * argument of the form -<letter> is seen, or whenever the argument is the last item on the
 * command line
 * (e.g. 'rlwrap -a command', which will be parsed as 'rlwrap --always-readline=command')
 */

static char *check_optarg(char opt, int remaining, bool isdummy, const char *suggestion)
{
    MAYBE_UNUSED(opt);
    MAYBE_UNUSED(remaining);
    MAYBE_UNUSED(isdummy);
    MAYBE_UNUSED(suggestion);

    if (!optarg)
        last_option_didnt_have_optional_argument =
            TRUE; /* if this variable is set, and  if
                     command is not found, suggest that it may have been meant as optional argument (e.g. 'rlwrap -a
                     password sqlplus' will try to execute 'password sqlplus' ) */
    ...
    if (optarg && /* is there an optional arg? have a look at it: */
        ...
        myerror(WARNING | NOERRNO,
                ...
                "Is this what you meant? If not, please provide %s  argument like '%s'",
                optarg, opt, isdummy ? "a dummy" : "an", suggestion);
#endif
...
char *read_options_and_command_name(int argc, char **argv)
{
    int   c;
    char *opt_C = NULL;
    int   option_count = 0;
    ...
    while (1) {
        ...
        switch (c) {
            ...
            case 'A':
                ...
                if (check_optarg('A', remaining, TRUE, "") && strings_are_equal(optarg, "!")) bleach_the_prompt = TRUE;
                break;
            case 'b':
            ...
            case 'c':
                ...
                myerror(WARNING | NOERRNO,
                        "On this system rlwrap cannot follow the rlwrapped command's working directory:\n"
            ...
            case 'd':
                ...
                if (option_count > 1)
                    myerror(FATAL | NOERRNO,
                            "-d or --debug option has to be the *first* rlwrap option\n"
                ...
                my_fopen(&debug_fp, DEBUG_FILENAME, "w+", "debug log"); /* w+, not w: both parent
                        and child write to the same logfile, and need to fseek beyond where
                        the other may have written stuff */
#else
                // clang-format off
            ...
            case 'F':
                // clang-format off
                WONTRETURN(myerror(FATAL | NOERRNO, "The -F (--history-format) option is obsolete. Use -z \"history_format '%s'\" instead", optarg));
                // clang-format on
                break;
            case 'g':
                forget_regexp = mysavestring(optarg);
                match_regexp("complain NOW if regexp is wrong", forget_regexp, 1);
                break;
            case 'h':
                WONTRETURN(usage(EXIT_SUCCESS));
            case 'H':
                history_filename = mysavestring(optarg);
                break;
            case 'i':
                if (opt_f) myerror(FATAL | NOERRNO, "-i option has to precede -f options");
                completion_is_case_sensitive = FALSE;
                break;
            case 'I':
                pass_on_sigINT_as_sigTERM = TRUE;
                break;
            case 'l':
                open_logfile(optarg);
                break;
...
#endif
                multiline_separator = /* \\\\ will be printed as \\ which is correct if we want
                                        ' \ ' to be the multiline separator */
                    (check_optarg('m', remaining, FALSE, " \\\\ ") ? mysavestring(optarg) : " \\ ");
                break;
            case 'M':
                multi_line_tmpfile_ext = mysavestring(optarg);
                break;
            case 'N':
                commands_children_not_wrapped = TRUE;
                break;
            case 'o':
                one_shot_rlwrap = TRUE;
                break;
            case 'O':
                prompt_regexp = mysavestring(optarg);
                if (*prompt_regexp == '!') {
            ...
            case 'p':
                ...
                initialise_colour_codes(check_optarg('p', remaining, FALSE, "Red") ? colour_name_to_ansi_code(optarg)
            ...
            case 's':
                ...
                break;
            case 'S':
                substitute_prompt = mysavestring(optarg);
                break;
            case 't':
            ...
            case 'X':
                ...
                break;
            case 'z':
                filter_command = mysavestring(optarg);
                break;
            case '?':
                // clang-format off
                assert(optind > 0);
    ...
    if (!complete_filenames && !opt_b) { /* use / and . as default breaking characters whenever we
                                            don't complete filenames */
        rl_basic_word_break_characters = add2strings(rl_basic_word_break_characters, "/.");
    }
    ...
    if (opt_C) {
        ...
        if (countback > 0) {                 /* e.g -C 1 or -C 12 */
            if (argc - countback < optind) { /* -C 666 */
                myerror(FATAL | NOERRNO, "when using -C %d you need at least %d non-option arguments", countback,
                        countback);
            } else if (argv[argc - countback][0] == '-') { /* -C 2 perl -d blah.pl */
                myerror(FATAL | NOERRNO, "the last argument minus %d appears to be an option!", countback);
...
int output_queue_is_nonempty(void) { return (output_queue ? TRUE : FALSE); }
...
void flush_output_queue(void)
{
    ...
    if (!output_queue) return;
    queuelen = strlen(output_queue);
    nl = strchr(output_queue, '\n');
    how_much = min(BUFFSIZE, nl ? 1 + nl - output_queue : queuelen); /* never write more than one
        line, and never more than BUFFSIZE in one go */
    ...
    if (debug) {
        ...
        output_queue[nwritten] = '\0'; /* temporarily replace the last written byte + 1 by a '\0' */
                                       // clnag-format off
        DPRINTF3(DEBUG_TERMIO, "flushed %d of %d bytes from output queue to pty: %s", nwritten, queuelen,
                 M(output_queue));
        // clang-format on
...
void cleanup_rlwrap_and_exit(int status)
{
    ...
    if (write_histfile && (histsize == 0 || history_total_bytes() > 0)) { /* avoid creating empty .
            speling_eror_history file after typo */
    ...
    mymicrosleep(10); /* we may have got an EOF or EPIPE because the filter or command died, but this doesn't mean that
                         SIGCHLD has been caught already. Taking a little nap now improves the chance that we will catch
                         it (no grave problem if we miss it, but diagnostics, exit status and transparent signal
    ...
    if (filter_pid)
        ...
        myerror(WARNING | NOERRNO,
				(filters_killer ? "filter was killed by signal %d (%s)" :
					WEXITSTATUS(filters_exit_status) ? "filter died" : "filter exited"),
				filters_killer,
				signal_name(filters_killer)
				);
        // clang-format on
    }
    if (debug) debug_postmortem();

    if (bracketed_paste_enabled) {
        int saved_nl_came_last = newline_came_last;
        DPRINTF0(DEBUG_READLINE, "disabling bracketed-paste");
        my_putstr(term_disable_bracketed_paste); /* this will not output a newline, but also not add any visible output,
                                                    so ... */
