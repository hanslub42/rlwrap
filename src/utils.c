...
static FILE *log_fp;
/* Give up the processor so that other processes (like command) can have their say. Especially
   useful with multi-line input: if we, after writing a line to the inferior command, would return
   to the main select()loop immediately, we would find more output to write, and keep blathering
   away without giving the poor command time to say something back */

void yield(void)
{
    DPRINTF0(DEBUG_TERMIO, "yield...");
#if 0 /* If command writes some output and immediately starts a long computation, \
         sched_yield () will not return for a long time,and rlwrap will not see   \
         the output for all that time. The "poor mans sched_yield" with select    \
...
void zero_select_timeout(void) { signal_handled = TRUE; }

#else

#    define until_hell_freezes NULL;
static struct timeval *volatile pmy_select_timeout_tv; /* The SIGCHLD handler sets this variable
        (see zero_select_timeout() below) to make select() return immediately when a child has
        died. gcc (4.8 and higher) may optimize it into a register, which won't work: hence the
        "volatile" keyword */
...
struct termios *my_tcgetattr(int fd, char *UNUSED(which))
{
    struct termios *pterm = (struct termios *)mymalloc(sizeof(struct termios));
    return tcgetattr(fd, pterm) < 0 ? NULL : pterm;
}

/* empty signal handler */
void do_nothing(int UNUSED(signal)) { /* yawn.... */ }
        ...
        while (1) {
            ...
            if ((nread = read(fd, (char *)buffer + total_read, count - total_read)) <= 0) {
                if (nread < 0 && errno == EINTR) {
                    if (interruptible) myerror(FATAL | NOERRNO, "(user) interrupt reading %s, filter_hangs?", whence);
    ...
    while (1) {
        if ((nwritten = write(fd, (char *)buffer + total_written, count - total_written)) <= 0) {
            if (nwritten < 0 && errno == EINTR) {
                if (interruptible) myerror(FATAL | NOERRNO, "(user) interrupt - filter hangs?");
                if (received_sigALRM) {
                    received_sigALRM = FALSE;
                    interruptible = TRUE;
                }
...
void set_ulimit(int resource, long value)
{
    ...
    // clang-format off
    DPRINTF4(DEBUG_ALL,
		"setrlim() used to set resource #%d to value %ld, result = %d (%s)",
		resource,
		value,
...
static char *markup(const char *colour_name, const char *str)
{
...
void myerror(int error_flags, const char *message_format, ...)
{
    ...
    if (!(is_warning && nowarn)) fputs(message, stderr); /* @@@ error reporting (still) uses buffered I/O */
...
void my_fopen(FILE **pfp, const char *path, const char *mode, const char *description)
{
    ...
    *pfp = fopen(path, mode);
    if (!*pfp) myerror(FATAL | USE_ERRNO, "Cannot %s  %s %s for %s", what, description, path, how);
}
...
size_t filesize(const char *filename)
{
    ...
    if (stat(filename, &buf)) myerror(FATAL | USE_ERRNO, "couldn't stat file %s", filename);
...
void close_open_files_without_writing_buffers(void) /* called from child just before exec(command) */
{
    if (log_fp) close(fileno(log_fp)); /* don't flush buffers to avoid avoid double double output output */
    if (debug) close(fileno(debug_fp));
}
...
*/
static int get_new_slave_cwd(char **cwd)
{
    ...
    if (!proc_pid_cwd) proc_pid_cwd = add3strings(PROC_MOUNTPOINT, "/", add2strings(as_string(command_pid), "/cwd"));

#    ifdef HAVE_READLINK
    if (readlink(proc_pid_cwd, readlink_buffer, MAXPATHLEN) > 0) possibly_new_cwd = mysavestring(readlink_buffer);
#    else
...
/* change_working_directory() tries to change rlwrap's working directory to the rlwrapped command's
   current working directory */
void change_working_directory(void)
{
    static char *slaves_working_directory = NULL;
    if (!slaves_working_directory) slaves_working_directory = mysavestring(".");
...
{
    if (!terminal_settings) return;
    DPRINTF3(DEBUG_TERMIO, "terminal settings: clflag.ISIG: %s, cc_c[VINTR]=%d, cc_c[VEOF]=%d",
             isset(terminal_settings->c_lflag | ISIG), terminal_settings->c_cc[VINTR], terminal_settings->c_cc[VEOF]);
}
...
static void print_option(char shortopt, char *longopt, char *argument, int optional, char *comment)
{
    ...
    if (!long_opts) longopt = search_and_replace("-", " ", longopt, 0, NULL, NULL);
...
static void print_debug_flag(int flag, char *explanation) { fprintf(stderr, "    %4d    %s\n", flag, explanation); }
...
void usage(int status)
{
    ...
    print_option('X', "skip-setctty", NULL, FALSE, NULL);
    ...
    fprintf(stderr,
            "    default debug mask = %d (debug termio, signals and readline handling)\n"
...
void mysetsid(void)
{
    ...
    DPRINTF2(DEBUG_TERMIO, "setsid() returned %d %s", (int)ret, ERRMSG(ret < 0));
#endif
}
